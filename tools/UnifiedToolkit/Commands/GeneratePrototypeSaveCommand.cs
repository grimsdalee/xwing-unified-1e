...
public static class GeneratePrototypeSaveCommand
{
        ...
        return new JsonObject
        {
            ...
            ["CustomMesh"] = new JsonObject
            {
                ["MeshURL"] =
                    ...
                    "xwing-unified-1e/main/assets/source/unified1e/" +
                    "runtime/tokens/flipToken.obj",
                ["DiffuseURL"] =
        ...
        return new JsonObject
        {
            ...
            ["CustomMesh"] = new JsonObject
            {
                ...
                ["ColliderURL"] =
                    "https://raw.githubusercontent.com/grimsdalee/" +
                    "xwing-unified-1e/main/assets/source/unified1e/" +
                    "support/models/minisculebox.obj",
                ["Convex"] = true,
                ["MaterialIndex"] = 1,
                ["TypeIndex"] = 1,
                ["CastShadows"] = true
    ...
    private static readonly Dictionary<
        string,
        IReadOnlyList<AssetPathMigration>>
        Unified1eMigrationCache = new(
            StringComparer.OrdinalIgnoreCase);

    private static string ResolveUnified1eMigratedPath(
        string repositoryRoot,
        string sourcePath)
    {
        var normalisedSource = sourcePath
            .Replace('\\', '/')
            .TrimStart('/');

        if (normalisedSource.StartsWith(
        ...
        if (!Unified1eMigrationCache.TryGetValue(
                repositoryRoot,
                out var migrations))
        {
            migrations = LoadUnified1eAssetPathMigrations(
                repositoryRoot);
            Unified1eMigrationCache[repositoryRoot] = migrations;
        }

        foreach (var migration in migrations)
        {
            if (normalisedSource.Equals(
                    migration.SourcePath,
                    StringComparison.OrdinalIgnoreCase))
            {
                return migration.DestinationPath;
            }

            var sourcePrefix =
                migration.SourcePath.TrimEnd('/') + "/";
            ...
            return migration.DestinationPath.TrimEnd('/')
                + "/"
                + normalisedSource[sourcePrefix.Length..];
        }

        throw new InvalidDataException(
            $"No approved Phase 13 migration mapping exists for " +
            $"ship asset '{normalisedSource}'.");
    }

    private static IReadOnlyList<AssetPathMigration>
        LoadUnified1eAssetPathMigrations(
            string repositoryRoot)
    {
        ...
        ValidateFile(
            planPath,
            "Phase 13 Unified 1E asset migration plan");
        ...
        {
            throw new InvalidDataException(
                "The Phase 13 asset migration plan has no entries array.");
        }

        var migrations = new List<AssetPathMigration>();
        ...
        foreach (var entry in entries.EnumerateArray())
        {
            ...
            destination = destination.Replace('\\', '/').TrimStart('/');
            ...
            migrations.Add(new AssetPathMigration(
                source,
                destination));
        }
        ...
        return migrations
            .OrderByDescending(
                migration => migration.SourcePath.Length)
            .ToList();
    }
        ...
        ICollection<string> diagnostics)
    {
                     ...
                     StringComparison.OrdinalIgnoreCase))
        {
            ...
            RecordShipModelSelectionAudit(
                repositoryRoot,
                assembly,
                "assets/source/unified1e/ships/small/z95headhunter/Z95.obj",
                relative,
            ...
            RecordShipModelSelectionAudit(
                repositoryRoot,
                assembly,
                "assets/source/unified1e/ships/small/z95headhunter/Z95v2.obj",
                relative,
                "Unified 2.5 spawned validation save confirms Z95v3.obj is the production mesh.",
                "Obsolete");
        }
        ...
        relative = ResolveUnified1eMigratedPath(
            repositoryRoot,
            relative);
...
public sealed record AssetPathMigration(
    string SourcePath,
