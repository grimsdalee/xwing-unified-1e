...
public static class BuildFirstEditionShieldUpgradeValidationCommand
{
    ...
    public static int Run(string[] args)
    {
        ...
        try
        {
            ...
            var result = new FirstEditionShieldUpgradeValidationBuilder().Build(repository, sourceSave, request,
            ...
            var output = Path.GetFullPath(Option(args, "--output") ?? Path.Combine(repository,
                "_unifiedtoolkit_reports", "phase16", "shield-upgrade-validation"));
            Directory.CreateDirectory(output);
            const string stem = "redsquadronpilot-shield-upgrade-physical-resource-v1";
            ...
            Console.WriteLine("UnifiedToolkit Phase 16F-R10 Shield Upgrade Validation");
            Console.WriteLine("========================================================");
            Console.WriteLine();
            ...
            Console.WriteLine($"Existing active shields:   {result.Manifest.ActiveExistingShieldCount}");
            Console.WriteLine($"Final active shields:      {result.Manifest.ActiveFinalShieldCount}");
            Console.WriteLine($"Control shields:           {result.Manifest.ControlShieldCount}");
            Console.WriteLine($"Added shield-token GUID:   {result.Manifest.ShieldToken.TokenGuid}");
            ...
            Console.WriteLine();
            Console.WriteLine("Shield Upgrade validation prepared. One additional owned shield token is active; the control ship is unchanged.");
            return result.Manifest.IsValid ? 0 : 2;
        }
        catch (Exception exception)
        {
            Console.Error.WriteLine($"First Edition Shield Upgrade validation failed: {exception.Message}");
            return 1;
        }
    }

    private static IEnumerable<string> Report(FirstEditionShieldUpgradeValidationManifest manifest)
    {
        yield return "# First Edition Shield Upgrade Validation";
        yield return "";
        ...
        yield return $"- Added shield token: `{manifest.ShieldToken.TokenGuid}`";
        yield return $"- Active shield row: **{manifest.ActiveExistingShieldCount} → {manifest.ActiveFinalShieldCount}**";
        yield return $"- Control shield row: **{manifest.ControlShieldCount}**";
        yield return "- Printed stat overlays: **0**";
        yield return "";
    ...
    private static void Usage() => Console.WriteLine(
        "Usage: UnifiedToolkit build-first-edition-shield-upgrade-validation <repository> <tts-save.json> --pilot <id|name|import-id> --pilot-card-guid <guid> --control-pilot-card-guid <guid> [--output <folder>]");
}
